...
class ResetCooldownCommand : SubCommand {
    override fun build(): LiteralCommandNode<CommandSourceStack> {
        return Commands.literal("resetcooldown")
            .requires(Permissions.require("${ActionItems.MOD_ID}.command.resetcooldown", 2))
    ...
    companion object {
        fun execute(
            ...
            if (!ItemManager.resetCooldown(player, id)) {
                ctx.source.sendMessage(
                    Component.text("Failed to reset cooldown for player ${player.name.string}!")
            ...
            ctx.source.sendMessage(
                Component.text("Successfully reset cooldown for player ${player.name.string}!")
