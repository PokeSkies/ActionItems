...
object ItemManager {
    ...
    private fun onItemUse(player: ServerPlayer, itemStack: ItemStack): InteractionResult {
        val actionItem = isActionItem(itemStack) ?: return InteractionResult.PASS
    ...
    private fun registerEvents() {
        UseItemCallback.EVENT.register { p, level, hand ->
            val itemStack = p.getItemInHand(hand)
            if (level.isClientSide) return@register InteractionResultHolder.pass(p.getItemInHand(hand))
            val player = p as ServerPlayer

            return@register InteractionResultHolder(onItemUse(player, itemStack), itemStack)
        }

        UseBlockCallback.EVENT.register { p, level, hand, _ ->
            if (level.isClientSide) return@register InteractionResult.PASS
            val player = p as ServerPlayer
            val itemStack = player.getItemInHand(hand)

            return@register onItemUse(player, itemStack)
        }
    }
}
